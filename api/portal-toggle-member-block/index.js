...
function buildBlockStatusEmailHtml(blocked) {
  const title = blocked ? "Tài khoản của bạn đã bị khoá" : "Tài khoản của bạn đã được mở lại";
  const color = blocked ? "#b91c1c" : "#15803d";
  const bodyText = blocked
    ? "Tài khoản của bạn tại Mạng Lưới Tri Thức Việt Nam đã bị tạm khoá và không thể đăng nhập. Nếu bạn cho rằng đây là nhầm lẫn, vui lòng liên hệ với chúng tôi để được hỗ trợ."
    : "Tài khoản của bạn tại Mạng Lưới Tri Thức Việt Nam đã được mở lại. Bạn có thể đăng nhập bình thường trở lại.";
      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 28px rgba(15,23,42,0.10);">
          ...
          <tr><td style="padding:32px;">
            ...
            <p style="font-size:14px;color:#334155;margin:0 0 20px;line-height:1.6;">${bodyText}</p>
    ...
    if (apiKey && fromEmail) {
      try {
        ...
        await sgMail.send({
          ...
          subject: blocked ? "Tài khoản của bạn đã bị khoá" : "Tài khoản của bạn đã được mở lại",
          html: buildBlockStatusEmailHtml(blocked)
        });
      } catch (err) {
